...
class connection (addr, port)=
    ...
    object(self : 'a)
        ...
        method run (func: 'a -> in_channel -> out_channel -> unit)  = 
            ...
            with _ -> Printf.printf "Silent fail of connection\n"

        method stop () = match ic with None -> Printf.printf "Nothing to close !\n" | Some icc -> Unix.shutdown_connection icc
                end;;


class ['a]  pool n=
    (** Gestion automatique d'un pool dobject qui effectuent des actions...*)
    object(self)
        val size = n
...
class ['a] threadedPool n=
    (** On met dans le pool des threads dobjets qui repondent aux methodes run/stop*)
    object(self)
        ...
        method tryClean ()=
            for i=0 to n-1 do
                try 
                    ...
                    if el#timeFromStart () > 10. then el#kill (); (*ici valeur fixe arbitraire, il faudrait la variabiliser TODO*)
...
module Action = struct
    ...
    let discovery_network my_addr port=
        ...
        let areYou connection ic oc=
            Printf.printf "la fonction areYou est executée \n"; flush stdout;
            try 
    ...
    let ask question range port= 
        ...
        let ask_server question connection ic oc=
            ...
            try 
                output_string oc ("KK"^question^"\n"); flush oc; (*KK is not knock knock*)
				let line = ref "" in
				while !line <> "END" do
					line := input_line ic;
...
(**Code executed by the client*)
	...
	(fun fileName -> 
		(*le top serait que les ips s'inscrivent en temps reel dans le fichier plutot que dattendre la fin du test pour ecrire tout TODO*)
	...
	(**Cette fonction nous permettrait d'utiliser la bonne adresse pour le serveur mais il
